...
//
// open-terminal-for-approval이 필요로 하는 computeUnapprovedDirs(main.ts:1463-1475) 판정은
// live_rows.rs의 compute_unapproved_dirs(refresh-board가 쓰는 그 함수, canonical)를 그대로
// 재사용한다 — 애초에 이 청크가 만들 때는 별도로 이관했었는데, 같은 로직이 두 곳에서 관리되며
// 드리프트될 위험이 있어(리뷰에서 지적) 여기서는 중복 정의를 없애고 import만 한다.
// ---------------------------------------------------------------------------------------------
...
pub async fn open_terminal_for_approval_command(target_dir: String) {
    let is_known_unapproved = crate::live_rows::compute_unapproved_dirs(&load_leads()).into_iter().any(|d| d.dir == target_dir);
...
mod tests {
    ...
    // compute_unapproved_dirs 관련 단위 테스트는 canonical 정의가 있는 live_rows.rs로 이동했다
    // (중복 제거) — 그 테스트들이 이 함수를 계속 커버한다.
